...
func newSysRole(db *gorm.DB, opts ...gen.DOOption) sysRole {
	...
	_sysRole.RequestID = field.NewString(tableName, "request_id")
...
type sysRole struct {
	...
	ALL         field.Asterisk
	ID          field.Int64
	Code        field.String // 角色编码(代码引用的稳定标识,不可变)
	Name        field.String // 角色名称(显示用)
	Description field.String // 角色描述
	IsBuiltin   field.Int32  // 内置角色: 1是(admin/hr/manager/employee,不可删除) 0否
	Status      field.Int32  // 状态: 1启用 0禁用(禁用后其授权即时失效)
...
func (s *sysRole) updateTableName(table string) *sysRole {
	...
	s.IsBuiltin = field.NewInt32(table, "is_builtin")
...
func (s *sysRole) fillFieldMap() {
	...
	s.fieldMap["is_builtin"] = s.IsBuiltin
...
func (s sysRoleDo) Create(values ...*db_model.SysRole) error {
	if len(values) == 0 {
		return nil
	}
...
func (s sysRoleDo) CreateInBatches(values []*db_model.SysRole, batchSize int) error {
	return s.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (s sysRoleDo) Save(values ...*db_model.SysRole) error {
	if len(values) == 0 {
		return nil
	}
...
func (s sysRoleDo) First() (*db_model.SysRole, error) {
...
func (s sysRoleDo) Take() (*db_model.SysRole, error) {
...
func (s sysRoleDo) Last() (*db_model.SysRole, error) {
...
func (s sysRoleDo) Find() ([]*db_model.SysRole, error) {
	result, err := s.DO.Find()
	return result.([]*db_model.SysRole), err
}

func (s sysRoleDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*db_model.SysRole, err error) {
...
func (s sysRoleDo) FindInBatches(result *[]*db_model.SysRole, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (s sysRoleDo) FirstOrInit() (*db_model.SysRole, error) {
...
func (s sysRoleDo) FirstOrCreate() (*db_model.SysRole, error) {
...
func (s sysRoleDo) FindByPage(offset int, limit int) (result []*db_model.SysRole, count int64, err error) {
...
func (s sysRoleDo) Delete(models ...*db_model.SysRole) (result gen.ResultInfo, err error) {
