...
--
-- 本文件包含三段相互独立的脚本，运行时由 Go 按 "-- __SCRIPT_SEPARATOR__"
-- 切分为三个脚本分别 EVAL。为避免三段被当作同一模块时产生"变量重复声明"
-- 以及"redis/cjson/KEYS/ARGV 未定义全局"的静态分析报错，每段都用 do ... end
-- 包裹成独立作用域，并显式捕获 Redis 在脚本执行期注入的全局（redis/cjson/KEYS/ARGV）。
-- 这些改动对 Redis 运行时语义无任何影响。
